...
class Processed_CSVDataset(Dataset):
    """CSV dataset."""
    def __init__(self, img_folder, ann_file, class_list, verb_path, role_path, verb_info, is_training, transform=None, neighbors_dict=None):
        """
        Parameters:
            - ann_file : CSV file with annotations
            - class_list : CSV file with class list
    ...
    def __getitem__(self, idx):
        ...
        neighbors_indices = [self.image_to_image_idx[i] for i in neighbors_names]
        # True Self
        img = self.load_image(idx)
        annot = self.load_annotations(idx)
        verb = self.image_names[idx].split('/')[2]
        ...
        for idx in neighbors_indices:
            ...
            verb_role = self.verb_info[verb]['order']
            verb_role_idx = [self.role_to_idx[role] for role in verb_role]
            sample.append({'img': img, 'annot': annot, 'img_name': self.image_names[idx], 'verb_idx': verb_idx, 'verb_role_idx': verb_role_idx})
        if self.transform:
            sample = [self.transform(sample[i]) for i in range(len(neighbors_indices)+1)]
        return sample
    def load_image(self, image_index):
        im = Image.open(self.image_names[image_index])
        im = im.convert('RGB')
...
def processed_collater(data):
    imgs, annots, shift_0, shift_1, scales, img_names, verb_indices, verb_role_indices = [], [], [], [], [], [], [], []
    for sample in data:
        for s in sample: 
            imgs.append(s['img'])
...
def processed_build(image_set, args, neighbors_dict=None):
    root = Path(args.swig_path)
    img_folder = root / "images_512"
