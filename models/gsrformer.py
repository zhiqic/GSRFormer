...
from .transformer import build_encoder_encoder as build_encoder_enc
from .transformer import build_encoder_decoder as build_encoder_dec
from .transformer import build_decoder_transformer as build_decoder


class Encoder_Transformer(nn.Module):
    """Encoder Part for GSRFormer"""
    def __init__(self, backbone, transformer1, transformer2, num_noun_classes, vidx_ridx):
    ...
    def forward(self, samples, targets=None, inference=False):
        ...
        batch_features, batch_role_tokens = torch.zeros(0, device=device), torch.zeros(0, device=device)
        # model prediction
        for i in range(batch_size):
            if not inference:
                ...
                tgt = None
            img_ft, verb_ft, verb_pred_1 = self.transformer1(self.input_proj(src[i:i+1]),
                                                           mask[i:i+1], pos[-1][i:i+1])
            
            verb_ft, noun_ft, role_tokens = self.transformer2(img_ft, verb_ft, verb_pred_1, 
                                                 mask[i:i+1], self.role_token_embed.weight,
                                                 pos[-1][i:i+1], self.vidx_ridx,
                                                 targets=tgt, inference=inference)
            bs = img_ft.shape[1]
            ...
            verb_pred_2 = self.verb_classifier(self.norm1(verb_ft.view(bs,-1)))
            ...
            batch_features = torch.cat((batch_features, torch.cat((verb_ft, noun_ft), dim=1)), dim=0)
            batch_role_tokens = torch.cat((batch_role_tokens, role_tokens), dim=0)
        # outputs
        out = {}
...
class Decoder_Transformer(nn.Module):
    ...
    def forward(self, batch_features, batch_role_tokens):
        ...
        for i in range(0, batch_size, step):
            verb_ft, noun_ft = self.transformer(batch_features[i:i+step, :, :],
    ...
    def forward(self, outputs, targets, eval=False):
        ...
        for i in range(batch_size):
            ...
            for n in range(NUM_ANNOTATORS):
                e_noun_loss.append(self.loss_function_noun(role_pred, role_targ[:, n].clone()))
...
def build_encoder_transformer(args):
    backbone = build_backbone(args)
    transformer1 = build_encoder_enc(args)
    transformer2 = build_encoder_dec(args)

    model = Encoder_Transformer(backbone, transformer1, transformer2,
                              num_noun_classes=args.num_noun_classes, vidx_ridx=args.vidx_ridx)
    criterion = None

    if not args.inference:
        weight_dict = {'loss_nce': args.noun_loss_coef, 'loss_vce_1': args.verb_loss_coef,
                       'loss_vce_2': args.verb_loss_coef, 'loss_bbox':args.bbox_loss_coef,
                       'loss_giou':args.giou_loss_coef, 'loss_bbox_conf':args.bbox_conf_loss_coef}
...
def build_decoder_transformer(args):
    ...
    model = Decoder_Transformer(transformer, args.num_noun_classes, args.vidx_ridx)
    criterion = None
    ...
    if not args.inference:
        ...
        if not args.test:
            criterion = SWiG_Criterion_Decoder(weight_dict=weight_dict, 
                                      SWiG_json_train=args.SWiG_json_train, 
                                      SWiG_json_eval=args.SWiG_json_dev,
                                      idx_to_role=args.idx_to_role)
        else:
